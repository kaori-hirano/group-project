...
## only select the portions where we need to need for our porject
...
```
  In this part, neraly all variables in wh_2023 are selected because we do not have sufficient background to interpret necessary relationships. We do the same for vdem_use dataset. However, incorporating all datasets is not possible. Thus, we incorporate all the quantified data variables measing political events and possible social activities. Thus the final incorporated is a comprehensive dataset based on full analysis of given codebooks.  
```{r clean data}
...
```
For the sake of completeness of the data, no extra changes are made. For further analysis, the unclear NA data are omitted. 
...
cs <- cs_full %>% subset(year == 2019) %>% select(-one_of("civil_war", "coup")) %>% 
  drop_na()
```
  The names of used variables are shortened for further analysis. The year after 2011 are specifically selected because we mainly focus on the change before and after pandemic year. We first wanted to leave the data between 2012 and 2019 to do time series. After multiple trials, we determine to concentrate on only pandemic year.
...
note: we'll want to mention why we are using 2019 here or in methods
...
note: hey! can you do a visualization between region and social support, as well as one between civil society participation and participation in democracy? or even civil society index as well. actually region and like as many things as possible would be great--I'm trying to understand the similarities between regions in the results so any way to compare the means of them in various important predictors or like any predictors would be swell
...
# need to say why most observations is best as base/ref

# creates new data frame for tree plotting with regions as numbers still
cs_num <- cs

# recodes region to be the region it represents rather than a num
...
x <- model.matrix(cspart ~ csrepress+v2x_partipdem+edu+corr+cs_index+
                    social_support+choices+gen+region, data = cs)[, -1]
y <- cs$cspart
```
...
rf_feature_importance <- tibble(Feature = rf_feature_names, '% Increase MSE' = rf_importance_scores)
...
```{r data-vis}
# need to find a way to represent the relationships we are interested in. 
# i'm not really sure how we do that...
# ohhh we can do all of the importance plots of predictors showing the ones that are most important by model
# actually are the tables I made enough? I feel like they show the predictors that we're interested in 
```
# Discussion
