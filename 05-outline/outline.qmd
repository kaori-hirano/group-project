...
# NEED TO ADD THE INTRODUCTION AND PROPOSAL HERE, THEN START THE DATA PART
...
ggplot(cs, aes(cspart, fill = region)) + 
  geom_histogram() + 
  labs(x = "Civil Participation", 
       title = "Civil Participation across regions", 
       fill = "Regions")

# The median and mode values of the outcome variable across regions seem to be
# approximately within the same vicinity so as long as we choose one of the 
# regions NOT with abnormal 

# Plot the distribution by region
ggplot(cs, aes(cspart)) + 
  geom_histogram() + 
  labs(x = "Civil Participation", 
       title = "Civil Participation across regions",
       y = "Count") + 
...
```{r visualize-region-diff-cspart}
...
We get the summary data on all the variables to check whether the scale of the variables used as predictors vary widely. They do vary in scale. This is why it is ideal to scale the data while doing Lasso and Ridge regression. We do not have to scale data in random forest model. 

Because we are interested in how civil society participation is different across regions, we also plot the distribution of different important social variables in our hypothesis against regions to see whether there are any strong correlation between our predictors and regions. 

We also plot out the most important variables in our hypothesis against the outcome variable to see whether there is a complex relationship or a liner one between them. 
