...
cs$region <- relevel(cs$region, ref = 5)
group_by(cs, region) %>% summarize(m = mean(cspart))
...
p1=ggplot(compare_data, aes(x=name, y=value))+
  ...
  labs(x="models",y="R2",title = "Comparing R-squared of all models")
p2=ggplot(compare_data, aes(x=name, y=mse_all))+
  geom_col()+
  coord_cartesian(ylim=c(0.0080,0.0095))+
  labs(x="models",y="mse",title = "Comparing MSE of all models")
p1+p2
```
